...
def test_expand_adds_neighbors_via_edges(tmp_path, monkeypatch):
    ...
    seed = [VectorHit(head_id="h1", distance=0.1, scope="global", head=HeadRecord(head_id="h1", body_id="b1", summary="h1"))]
...
def test_expand_caps_total_added(tmp_path, monkeypatch):
    ...
    seed = [VectorHit(head_id="h0", distance=0.1, scope="global", head=HeadRecord(head_id="h0", body_id="b1", summary="seed"))]
